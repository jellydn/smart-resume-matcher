...
export interface EditFieldParams {
  sectionType: "personalInfo" | "experience" | "education" | "projects" | "openSource";
  field: string;
  value: string;
  itemId?: string;
  ...
  return (
    <div className="space-y-4">
      <section>
        <h3 className="text-base font-semibold">
          {canEdit ? (
            <EditableText
              ...
              onSave={(value) =>
                onEditField?.({ sectionType: "personalInfo", field: "name", value })
              }
          ...
          <div className="text-xs mt-2 text-muted-foreground">
            {canEdit ? (
              <EditableText
                ...
                onSave={(value) =>
                  onEditField?.({ sectionType: "personalInfo", field: "summary", value })
                }
        ...
        {!isOriginal && getSuggestionsForSection("personalInfo", undefined, "summary").map((suggestion) => (
          <SuggestionInline
            key={suggestion.id}
            suggestion={suggestion}
          ...
          <section>
            ...
            <div className="space-y-3">
              {resume.experience.map((exp) => {
                const expSuggestions = getSuggestionsForSection("experience", exp.id);
                return (
                  <div key={exp.id}>
                    <div>
                        ...
                        {canEdit ? (
                          <>
                            <EditableText
                              value={exp.title}
                              onSave={(value) =>
                                onEditField?.({ sectionType: "experience", field: "title", value, itemId: exp.id })
                              }
                            ...
                            <EditableText
                              value={exp.company}
                              onSave={(value) =>
                                onEditField?.({ sectionType: "experience", field: "company", value, itemId: exp.id })
                              }
                              placeholder="Company"
                            />
                          </>
                      ...
                      <div className="text-xs text-muted-foreground mt-1">
                        {canEdit ? (
                          <EditableText
                            ...
                            onSave={(value) =>
                              onEditField?.({ sectionType: "experience", field: "description", value, itemId: exp.id })
                            }
                          ...
                          <li key={idx}>
                            {canEdit ? (
                              <EditableText
                                value={highlight}
                                onSave={(value) =>
                                  onEditField?.({ sectionType: "experience", field: "highlights", value, itemId: exp.id, highlightIndex: idx })
                                }
                    ...
                    {!isOriginal && expSuggestions.map((suggestion) => (
                      <SuggestionInline
                        key={suggestion.id}
                        suggestion={suggestion}
                        onAccept={onAcceptSuggestion ? () => onAcceptSuggestion(suggestion) : undefined}
          ...
          <section>
            ...
            <div className="space-y-2">
              {resume.education.map((edu) => {
                const eduSuggestions = getSuggestionsForSection("education", edu.id);
                return (
                  <div key={edu.id}>
                    ...
                    {!isOriginal && eduSuggestions.map((suggestion) => (
                      <SuggestionInline
                        key={suggestion.id}
                        suggestion={suggestion}
                        onAccept={onAcceptSuggestion ? () => onAcceptSuggestion(suggestion) : undefined}
              ...
              {resume.skills.map((skill) => {
                ...
                return (
                  <Badge
                    ...
                    className={cn(
                      "text-xs",
                      !isOriginal && isMatched && "bg-green-600 hover:bg-green-700"
                    )}
                  >
                    {skill.name}
          ...
          <section>
            ...
            <div className="space-y-2">
              {resume.projects.map((proj) => {
                const projSuggestions = getSuggestionsForSection("projects", proj.id);
                return (
                  <div key={proj.id}>
                    <p className="font-medium text-sm">
                      {canEdit ? (
                        <EditableText
                          value={proj.name}
                          onSave={(value) =>
                            onEditField?.({ sectionType: "projects", field: "name", value, itemId: proj.id })
                          }
                      ...
                      <div className="text-xs text-muted-foreground">
                        {canEdit ? (
                          <EditableText
                            ...
                            onSave={(value) =>
                              onEditField?.({ sectionType: "projects", field: "description", value, itemId: proj.id })
                            }
                    ...
                    {!isOriginal && projSuggestions.map((suggestion) => (
                      <SuggestionInline
                        key={suggestion.id}
                        suggestion={suggestion}
                        onAccept={onAcceptSuggestion ? () => onAcceptSuggestion(suggestion) : undefined}
          ...
          <section>
            ...
            <div className="space-y-2">
              {resume.openSource.map((os) => {
                const osSuggestions = getSuggestionsForSection("openSource", os.id);
                return (
                  <div key={os.id}>
                    <p className="font-medium text-sm">
                      {canEdit ? (
                        <>
                          <EditableText
                            value={os.project}
                            onSave={(value) =>
                              onEditField?.({ sectionType: "openSource", field: "project", value, itemId: os.id })
                            }
                            placeholder="Project Name"
                          />
                          {" "}
                      ...
                      <div className="text-xs text-muted-foreground">
                        {canEdit ? (
                          <EditableText
                            ...
                            onSave={(value) =>
                              onEditField?.({ sectionType: "openSource", field: "description", value, itemId: os.id })
                            }
                    ...
                    {!isOriginal && osSuggestions.map((suggestion) => (
                      <SuggestionInline
                        key={suggestion.id}
                        suggestion={suggestion}
                        onAccept={onAcceptSuggestion ? () => onAcceptSuggestion(suggestion) : undefined}
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className={cn("flex items-center gap-1 text-sm font-medium", getScoreColor(matchScore))}>
