...
import { ArrowLeft, Briefcase, X, ArrowRight, Loader2, AlertCircle, Sparkles, FileDown } from "lucide-react";
import { Link } from "react-router";
import type { JobDescription, JobRequirements, TailoringResult, Suggestion } from "~/lib/types";
...
import { ResumeComparisonView, type EditFieldParams } from "~/components/resume/resume-comparison-view";
import { applySuggestionToResume, revertSuggestionFromResume } from "~/lib/apply-suggestion";
...
export default function JobPage() {
  ...
  const { history, addEntry, deleteEntry, isLoaded: isHistoryLoaded } = useJobHistory();
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    const result = await parseJobDescription(jobDescription.description, settings);
  ...
  const handleSelectFromHistory = useCallback(
    (selectedJobDescription: JobDescription, selectedRequirements?: JobRequirements) => {
  ...
  const handleAcceptSuggestion = useCallback((suggestion: Suggestion) => {
    ...
    setTailoringResult({ ...tailoringResult, suggestions: updatedSuggestions });
  ...
  const handleEditField = useCallback((params: EditFieldParams) => {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      await exportResumeAsPdf(resume, requirements?.title, requirements?.company);
    } finally {
  ...
  const handleExportDocx = useCallback(async () => {
    ...
    try {
      await exportResumeAsDocx(resume, requirements?.title, requirements?.company);
    } finally {
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-between pt-4">
              <Button
                variant="outline"
                onClick={handleClear}
                disabled={(!hasContent && !jobDescription.linkedinUrl) || isAnalyzing || isTailoring}
              >
          ...
          <>
            ...
            {!hasResume && (
              <Alert>
                ...
                <AlertDescription>
                  Please <Link to="/resume" className="underline text-primary">create or upload your resume</Link> first to tailor it for this job.
                </AlertDescription>
              </Alert>
            )}
                ...
                <CardContent className="pt-6">
                  <div className="flex flex-col items-center gap-4">
                    <p className="text-muted-foreground text-center">
                      Ready to tailor your resume to match this job's requirements?
                    </p>
                    <Button
                      size="lg"
          ...
          <>
            <Card>
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold">Export Your Tailored Resume</h3>
