...
export function createResumeDocx(resume: Resume): Document {
	...
	if (personalInfo.email) {
		contactParts.push(
			new TextRun({ text: personalInfo.email, size: 18, color: "666666" }),
		);
	}
	if (personalInfo.phone) {
		...
		contactParts.push(
			new TextRun({ text: personalInfo.phone, size: 18, color: "666666" }),
		);
	}
	if (personalInfo.location) {
		...
		contactParts.push(
			new TextRun({ text: personalInfo.location, size: 18, color: "666666" }),
		);
	}
	if (personalInfo.linkedin) {
		...
		contactParts.push(
			new ExternalHyperlink({
				...
				children: [
					new TextRun({ text: "LinkedIn", size: 18, color: "2563eb" }),
				],
			}),
		);
	}
		...
		for (const exp of experience) {
			...
			children.push(
				new Paragraph({
					...
					tabStops: [
						{ type: AlignmentType.RIGHT, position: convertInchesToTwip(6.5) },
					],
	...
	if (skills.length > 0) {
		...
		const skillTexts = skills.map(
			(s) => `${s.name} (${proficiencyLabel(s.proficiency)})`,
		);
		children.push(
			new Paragraph({
				children: [
			...
			if (proj.url) {
				...
				projectChildren.push(
					new ExternalHyperlink({
						link: proj.url,
						children: [
							new TextRun({ text: "(link)", size: 18, color: "2563eb" }),
						],
					}),
				);
			}
